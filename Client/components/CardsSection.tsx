...
interface Product {
  ...
  arenaHalls: [{ id: number; name: string; nameAr: string; image: string }];
}
function CardsSection() {
  ...
  return loading ? (
    ...
    <>
      ...
      <div className={`my-10 px-4 sm:px-10 md:px-20 lg:px-32`}>
        <h2 className="text-2xl text-gray-500 font-bold">
          <span className="text-blue-400 font-bold  ">ARENA </span>Events
        </h2>
        <Swiper
          dir="ltr"
          className=""
        ...
        >
          {/* first Products */}
          {productsData && productsData.arenaHalls.length > 0 ? (
            productsData.arenaHalls.map((item: any, i: number) => (
              <SwiperSlide key={i}>
                <Link
                  href={`/arena/${item.id}`}
                  className="group relative flex flex-col items-center p-4 transition-all duration-300 hover:scale-[1.02] pb-8"
                >
                  {item.state == 1 ? (
                    ...
                    <span
                      className="absolute top-2 right-2 z-20 bg-gradient-to-r from-blue-600 to-blue-500 
                  px-4 py-1.5 rounded-full text-sm font-semibold text-white shadow-lg 
                  transform -rotate-2 hover:rotate-0 transition-transform duration-300
                  border-2 border-blue-400/20 backdrop-blur-sm"
                    >
