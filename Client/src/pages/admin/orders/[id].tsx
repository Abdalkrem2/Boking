...
function Profile() {
  ...
  useEffect(() => {
    if (db_link && session?.user?.token) {
      ...
      const fetchData = () => {
        const url = `${db_link}api/newCodes/${orderId}`;
        axios
          .get(url, {
            headers: {
  ...
  const updateHandler = (e: any) => {
    ...
    axios
      .post(`${db_link}api/orders/state/${orderId}`, fData, {
        headers: {
          Authorization: `Bearer ${session?.user.token}`,
        },
      })
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="p-6">
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="p-6 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors space-y-4">
                  <div className="border-b pb-3">
                    <p className="text-sm text-gray-500 mb-1">{t.cardNumber}</p>
                    <p className="text-lg font-semibold text-gray-900 tracking-wider">
                      {data.cardNumber}
                    </p>
                  </div>
                  <div className="border-b pb-3">
                    <p className="text-sm text-gray-500 mb-1">{t.nameOnCard}</p>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="w-1/3">
                      <p className="text-sm text-gray-500 mb-1">CCV</p>
                      <p className="text-lg font-semibold text-gray-900">
                        {data.cardCCV}
                      </p>
                    </div>
