...
function ProductsControl() {
  ...
  useEffect(() => {
    ...
    if (!searching) {
      setIsLoading(true);
      const url = `${db_link}api/orders/admin?page=${page}`;
      axios
        .get(url, {
          headers: {
  ...
  useEffect(() => {
    ...
    if (searching) {
      ...
      setIsLoading(true);
      const url = `${db_link}api/orders/search?page=${pageSearch}`;
      axios
        .post(
          url,
          { value },
  ...
  const handleSearch = (e: any) => {
    ...
    setIsLoading(true);
    const url = `${db_link}api/orders/search?page=${page}`;
    axios
      .post(
        url,
        { value },
        ...
        <div>
          ...
          <div className="flex sm:justify-between sm:flex-row gap-5 my-4 mx-2 flex-col justify-center items-center">
            <div>
              ...
              <p className="text-secondary-text">{t.thisIsAllOrders}</p>
            </div>
            <form
